...
**Last-updated**: 2026-05-13 ~20:25 GMT+7 (Reverse SSH path opened: OpenSSH server enabled, aree-home pubkey installed in `administrators_authorized_keys` with strict ACL. aree-home can now `ssh toey0@rdlt` for remote process inspection.)
...
| **OpenSSH Server** | enabled 2026-05-13 (`Add-WindowsCapability -Online -Name OpenSSH.Server~~~~0.0.1.0` + `Start-Service sshd` + auto-start + firewall rule on port 22). Auth: aree-home pubkey installed in `C:\ProgramData\ssh\administrators_authorized_keys` (admin path — toey0 is in BUILTIN\Administrators), ACL hardened to SYSTEM:F + Administrators:F only. Enables `ssh toey0@rdlt` from aree-home for remote process inspection / debugging. |
| arduino-cli | 1.4.1 (`C:\Program Files\Arduino CLI\arduino-cli.exe`) |
...
- **2026-05-13 ~19:48 GMT+7** — Toey reported "วันนี้เปิดคอมมาแล้วเล่นแค่เว็บแต่มันดูช้ามากเลย ค้างไปนานกว่าจะติด" while he was on the phone (away from RDLT until 19:00). Tailscale showed RDLT online (LAN direct 192.168.1.35:41641, healthy), but aree-home had no shell into RDLT — Phase 1 routes the *other* direction only. Pivoted to setting up a reverse SSH path.
- **2026-05-13 ~20:05 GMT+7** — OpenSSH Server feature installed via `Add-WindowsCapability -Online -Name OpenSSH.Server~~~~0.0.1.0`, `sshd` started, set Automatic startup, inbound firewall rule on port 22. Tailscale SSH server checkbox was tried first (Toey toggled "Run Tailscale SSH server" in tray Preferences) but `tailscale ssh toey@rdlt` returned `502 Bad Gateway, dial tcp 100.111.92.57:22: i/o timeout` — Windows version of Tailscale doesn't reliably ship a working SSH server, OpenSSH was the right tool.
- **2026-05-13 ~20:15 GMT+7** — aree-home pubkey (`ssh-ed25519 …FkoSwkd7iXIxiw4EQt1wCwtr+6+ou+78kw8zNx8uMF2 toey@aree-home`, fingerprint `SHA256:gj5EGNMYJFIFFWTOTbKWPvXqDmeZbwAYxiZETfC3Z/4`) appended to `C:\ProgramData\ssh\administrators_authorized_keys` (Windows OpenSSH admin path, since toey0 is in `BUILTIN\Administrators`). ACL hardened with `icacls /inheritance:r /grant Administrators:F /grant SYSTEM:F`. `Restart-Service sshd`. Two false starts before success: (a) tried `ssh toey@rdlt` and `ssh Toey@rdlt` — both `Invalid user` in `OpenSSH/Operational` event log → the real Windows username is `toey0` (zero suffix, surfaced via `Get-LocalUser`). (b) PowerShell multi-line paste from chat collapsed newlines into spaces — parser silently failed; recovery was paste-each-line-separately.
- **2026-05-13 ~20:25 GMT+7** — `ssh toey0@rdlt 'whoami'` from aree-home returned `rdlt\toey0`. Ran first remote diagnostic for the original slowness report: top CPU was `MsMpEng` (Microsoft Defender) at 906 sec accumulated, `Get-MpComputerStatus` showed AV signatures updated today 13:14, and `Get-HotFix` showed three Windows updates installed today (`KB5092762`, `KB5089549`, `KB5087051`). Conclusion: today's slowness is the expected post-update Defender verification scan (high disk I/O + CPU), normal pattern after a patch batch, resolves itself in 20–60 min. No malware, no failing hardware. Discussion with Toey about whether to permanently disable Defender — recommended A+B (schedule scans off-hours via `Set-MpPreference -ScanScheduleTime`, exclude dev/game folders via `Add-MpPreference -ExclusionPath`) over D (full disable via Tamper Protection off + Group Policy), since A+B keeps security and removes the actual pain. Toey deferred the decision.
...
- **2026-05-12 ~22:00 GMT+7** — Phase 3 closed: browser channel via ttyd + Tailscale Serve. (1) `apt install ttyd` (1.7.7) — note that this ALSO drops a system-level `ttyd.service` running as root with `/bin/login` shell, which silently grabbed port 7681 and broke the user service; fixed by `sudo systemctl disable --now ttyd.service` + `mask`. (2) User systemd unit `~/.config/systemd/user/aree-web.service` launches `ttyd --writable --port 7681 --interface 127.0.0.1 tmux new-session -A -s aree` — bind to localhost only so the only path in is via Tailscale Serve. (3) `sudo tailscale serve --bg http://127.0.0.1:7681` — required first-time enabling Serve feature at `https://login.tailscale.com/f/serve?node=…` by Toey in browser admin; once enabled the config activated retroactively. (4) HTTPS endpoint live at `https://aree-home.tail9e69b1.ts.net/` with Tailscale-issued cert; from aree-home self-test returned `HTTP/2 200`. (5) **Windows DNS adoption gotcha on RDLT**: `tailscale set --accept-dns=true` + `tailscale down/up` + `ipconfig /flushdns` did NOT make Windows resolve `aree-home.tail9e69b1.ts.net` — Windows kept using ISP IPv6 DNS (`2001:fb0:100::207:49`). Workaround was static entry in `C:\Windows\System32\drivers\etc\hosts` (`100.77.60.57 aree-home.tail9e69b1.ts.net`). Browser reached the URL after that. nslookup still times out because nslookup bypasses hosts file; the browser path uses the Windows DNS resolver chain which honors hosts.
- **2026-05-13 ~08:33 GMT+7** — Timezone switched from `Etc/UTC` (Ubuntu installer default) → `Asia/Bangkok` via `sudo timedatectl set-timezone Asia/Bangkok`. Reason: Aree was repeatedly mis-quoting times in retros and replies because `date` / `recap.ts` printed UTC and the human-readable conversion to GMT+7 was being missed. Switching the system clock to Asia/Bangkok makes `date`, journalctl, git commit timestamps, recap scripts, and shell prompts all render in Toey's local time natively — Aree no longer needs to mentally add 7 hours on every read. UTC is still recoverable via `date -u` if anything needs it. **Process note**: Toey flagged the drift directly ("คุณชอบเวลาผิดตลอดเลย แก้หน่อย") — the structural fix at the timezone layer is preferred over a per-script `TZ=Asia/Bangkok` wrapper because every future script inherits the right time without intervention.
